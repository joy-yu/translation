$(function() {
    ...
    function getSelectedText(target) {
        ...
        function getTrans(text) {
            ...
            var url = encodeURI(protocol + "//fanyi.youdao.com/openapi.do?keyfrom=microheart&key=1363430402&type=data&doctype=json&version=1.1&q=" + text);
            $.ajax({
                url: url,
                type: "GET",
                dataType: "JSON",
    ...
    $(document.documentElement).on("mouseup", getSelectedText)
                               .on("mousedown", rmTransDiv);
});